function [f,TotalInvRed,TotalR1Reqd,profit] = SKS_ProductionPlanning_IC(x,maxProfit)
...
penalty_profit = 0;
if profit<maxProfit
   penalty_profit = (profit-maxProfit)^2; 
end
...
for i = idx+1:length(x)
    if x(i)~=0
        if product(i)==product(idx)
             penalty_process = penalty_process+10^5;
        end
        idx = i;
    end
end
f = TotalInvRed + 10^15*(penalty_IC + penalty_R1 + penalty_R2 + sum(penalty_domain) + penalty_profit+penalty_process);   % Determining the fitness function value