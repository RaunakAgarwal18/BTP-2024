...
% Proposed difference vectors Lévy-mutation 
for i=1:Nc;
    if Nc<6; I=randperm(Nc);  II=find(I ~= i);I=I(II(1:3));
    Lights1(i,mcol)=Lights(I(1),mcol)+L(i,:,1).*(Lights(I(2),mcol)-Lights(I(3),mcol));%“DE/rand/1”
    else
    I=randperm(Nc);  II=find(I ~= i);I=I(II(1:5));%mating vectors (donners)
    Lights1(i,mcol)=Lights(I(1),mcol)+(L(i,:,1).*(Lights(I(2),mcol)-Lights(I(3),mcol))+L(i,:,2).*(Lights(I(4),mcol)-Lights(I(5),mcol)));%“DE/rand/2”:using Eq.(25) 
    end
end
...
Light_fitness1(i,:) = fobj(Lights1(i,:)); %#ok<AGROW>
