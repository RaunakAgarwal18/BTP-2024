...
for t=1:Max_iterations  % algorithm iteration
    ...
    pmm = 0.1 + 0.4 * (t / Max_iterations);
    nc = 5 + (20 - 5) * (t / Max_iterations);
    nm = 10 + (100 - 10) * (t / Max_iterations);
    ...
    for i = 1:size(offspring, 1)
        mutated_offspring(i, :) = pm(offspring(i, :), pmm, nm, lowerbound, upperbound);  % Mutation probability 0.3, nm = 20
    end
